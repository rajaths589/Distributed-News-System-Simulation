...
int main(int argc, char** argv) {
	...
	MPI_Comm_split(MPI_COMM_WORLD, color[world_rank], world_rank, &org_comm);
	...
	for(i = num_editors + 1; i < world_size; i++)
		org_color[i] = 0;
	MPI_Comm_split(MPI_COMM_WORLD, color[world_rank], world_rank, &news_source_comm);
